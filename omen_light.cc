...
Usage:
  ...
  led:   the led module to control. Can be ['front', 'case', 'cpu', 'fan_bot', 'fan_mid', 'fan_top']
...
# Turn off the case led when the power is on.
$ omen_light case on off
...
bool ParseLed(const std::string &arg, Report::Led *led)
{
  if (arg == "front")
    ...
    *led = Report::Led::FAN_BOT;
  else if (arg == "fan_mid")
    *led = Report::Led::FAN_MID;
  else if (arg == "fan_top")
    *led = Report::Led::FAN_TOP;
  else if (arg == "all")
    *led = Report::Led::ALL;
  else
  {
...
bool Run(int argc, char **argv)
{
  ...
  if (argc < 4)
    return false;
  if (!ParseLed(argv[1], &report.led))
    return false;
  if (!ParsePower(argv[2], &report.power))
    return false;
  if (!ParseMode(argv[3], &report.mode))
    return false;
  ...
  case Report::Mode::BLINKING:
    if (argc < 2)
      return false;
    if (!ParseSpeed(argv[0], &report.speed))
      return false;
    if (!ParseTheme(argv[1], &report.theme))
      return false;
    ...
    if (report.theme == Report::Theme::CUSTOM)
    {
      ...
      if (argc != 3 && argc != 6 && argc != 9 && argc != 12)
        return false;
